...
void print_row(Row* row) {
    std::cout << row->id << " " << row->username << " " << row->email <<endl;
}
...
void pager_flush(Pager* pager,uint32_t page_num,uint32_t size) {
    ...
    int* id = (int*) pager->pages[page_num];
...
void db_close(Table* table) {
    ...
    if (num_additional_rows > 0) {
        uint32_t pager_num = num_full_pages;
        if (pager->pages[pager_num] != NULL) {
            pager_flush(pager,pager_num,num_additional_rows*ROW_SIZE);
            free(pager->pages[pager_num]);
            pager->pages[pager_num] = NULL;
        }
    }
...
PrepareResult prepare_insert(string input_buffer,Statement* statement) {
    ...
    char * input = (char*)input_buffer.c_str();
    // std::cout << "here!!!!" << endl;
    // strcpy(input,input_buffer.c_str());
    
    char* keyword = strtok(input," ");
    char* id_string = strtok(NULL, " ");
    char* username = strtok(NULL, " ");
    char* email = strtok(NULL, " ");
    // std::cout << "here:" << keyword <<" " ;
    // std::cout <<"id:"<<id_string<<endl;
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        if (input_buffer[0] == '.') {
            MetaCommandResult r = do_meta_command(input_buffer,table);
            if (r == META_COMMAND_SUCCESS) {
                continue;
            }
            else if (r == META_COMMAND_UNRECOGNIZED_COMMAND) {
                std::cout << "Unrecognized command " << input_buffer << endl;
                continue;
            }
        }
        Statement statement;
